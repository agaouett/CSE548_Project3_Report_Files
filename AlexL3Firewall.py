...
class Firewall (EventMixin):
	...
	def __init__ (self,l2config,l3config):
		...
		# Alex Gaouette: Port Table for keeping track of MAC/IP Pairs
		self.portTable = dict()
	...
	def _handle_PacketIn(self, event):
		...
		# Alex Gaouette: DDOS Protection from IP Spoofing using the same MAC
		# All packets other than ARP
		else:
			# Add source MAC and apparent IP to table if match not found
			if(match.dl_src not in self.portTable):
				if(match.nw_src != None):
					self.portTable[match.dl_src] = match.nw_src
					print("Added MAC/IP pairing to table!")
					print("MAC: {} IP: {}".format(match.dl_src, self.portTable[match.dl_src]))
			# Check if IP and MAC are mismatched
			if(self.portTable.get(match.dl_src) == match.nw_src):
				# Original IP packet handling found in L3Firewall.py (Not written by Alex Gaouette)
				# This will process all IP packets that do not appear to be spoofed
				...
				# If MAC address does not match known IP address, drop the packets by creating a flow
				msg = of.ofp_flow_mod()
				# Filter the offending MAC address
				msg.match.dl_src = match.dl_src
				# An empty action list means the flow should be dropped, no need to specify
